#!/usr/bin/env python3
"""Build the Phase 0 encoder pretraining pool from EleutherAI/hendrycks_math.

Loads all 7 subject configs, filters to the requested difficulty levels (default
Level 1–4), extracts the boxed answer from each solution, and writes a JSONL with
the same schema used by all other pool files in this repo.

v2 design: default levels are 1–4 ONLY (Level 5 excluded for mutual exclusivity
with the Level 5 hard pool used for training and eval). Including Level 5 in the
easy pool would violate the pool separation constraint — the encoder would be
pretrained on some of the same problems it is later evaluated on.
